...
public class PackReadTest {
    ...
    void readsAndContainsExpectedValues(@TempDir Path tempDir) throws Exception {
        ...
        try (PrintWriter pw = new PrintWriter(temp)) { //add values to a temp file
            pw.println(3);
            pw.println(7);
            pw.println(11);
        }

        List<Card> cards = PackReader.readPack(temp.getAbsolutePath()); //read pack
        assertEquals(3, cards.size());// size and order are preserved
